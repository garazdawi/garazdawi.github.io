# `re`
[🔗](https://github.com/erlang/otp/blob/master/lib/stdlib/src/re.erl#L22)
...
The same as [`compile(Regexp,[])`](`compile/2`)

# `compile`

```erlang
-spec compile(Regexp, Options) -> {ok, mp() | exported()} | {error, ErrSpec}
                 when
...
# `replace(Subject, RE, Replacement) ≡ replace(Subject, RE, Replacement, [])`
