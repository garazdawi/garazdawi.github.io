# `diameter`
[🔗](https://github.com/erlang/otp/blob/master/lib/diameter/src/base/diameter.erl#L23)
...
-type app_module() ::
          ...
          #diameter_callback{peer_up :: term(),
                             peer_down :: term(),
                             pick_peer :: term(),
                             prepare_request :: term(),
                             prepare_retransmit :: term(),
                             handle_request :: term(),
                             handle_answer :: term(),
                             handle_error :: term(),
                             default :: term(),
...
```erlang
-type elapsed_time() ::
          {Hours :: non_neg_integer(), Mins :: 0..59, Secs :: 0..59, MicroSecs :: 0..999999}.
```
...
```erlang
-type eval() :: {module(), atom(), list()} | fun() | maybe_improper_list(eval(), list()).
```
...
```erlang
-type remotes() :: boolean() | [node()] | eval().
```
...
```erlang
-type restriction() :: false | node | nodes | [node()] | eval().
```
...
```erlang
-spec add_transport(SvcName, Transport) -> {ok, TRef} | {error, Reason}
                       when
...
```erlang
-spec call(SvcName, App, Request, CallOpts) -> Result
              when
                  SvcName :: service_name(),
                  App :: app_alias(),
...
```erlang
-spec remove_transport(SvcName, Pred) -> ok | {error, Reason}
                          when SvcName :: service_name(), Pred :: transport_pred(), Reason :: term().
```
...
```erlang
-spec service_info(SvcName, Item | [Item]) -> term()
                      when SvcName :: service_name(), Item :: atom() | peer_ref().
```
...
```erlang
-spec start_service(SvcName, Opts) -> ok | {error, Reason}
                       when SvcName :: service_name(), Opts :: [service_opt()], Reason :: term().
```
...
-spec which_connections() ->
                           ...
                           when
                               SvcName :: string(),
                               PeerInfo :: #{pid := pid(), uptime := elapsed_time()},
                               WDInfo ::
                                   #{ref := reference(),
                                     type := atom(),
                                     pid := pid(),
...
```erlang
-spec which_transports() -> [#{ref := reference(), type := atom(), service := string()}].
```
