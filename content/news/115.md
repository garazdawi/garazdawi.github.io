...
* A new tuple inÂ `crypto:supports/0`Â reports supported MAC
 algorithms.
* diameter:
 * Add service optionÂ `decode_format`Â to allow incoming messages to be decoded into maps
 instead of records.
...
* erts, kernel, tools: Profiling with lock counting can now be fully toggled at runtime in
 ...
 documentation on `lcnt:rt_mask/1`Â for details.
 
`lcnt:collect`Â and `lcnt:clear`Â will no longer block all
 other threads in the runtime system. This makes it possible to run the lock counting
 under heavier load.
* erts: The `zlib`Â module has been refactored and all its operations will now yield appropriately,
 allowing them to be used freely in concurrent applications.
* erts, tools: Add `erlang:iolist_to_iovec/1`, which converts an
 `iolist()`Â to an `erlang:iovec()`, which is suitable for use
...
* ssh: A new option `modify_algorithms`Â is implemented. It
...
* tools/xref: The predefined Xref analysis locals_not_used now understands
 the `-on_load()`Â attribute and does not report unused functions.
...
The Erlang/OTP source can also be found at GitHub on the official Erlang
 repository,Â [https://github.com/erlang/otp](https://github.com/erlang/otp)Â with tag OTP-20.1

The on-line documentation can be found at:Â [http://www.erlang.org/doc/](http://www.erlang.org/doc/)
